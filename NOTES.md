# Interview notes — concrete details per problem

The deliverable for each of the 7 core problems isn't just working code — it's being able to
explain it cold (README "Definition of done"). This file is that log. Problem definitions and
data-source descriptions live in `README.md`; Vue/Nuxt patterns in `CLAUDE.md` — **link, don't
restate.** Here, capture only the *specifics from this codebase*.

Fill one block per step as you finish it (build order in `CLAUDE.md`). Keep each answer to a
few sentences — if you can't write it tight, you don't understand it yet.

Template per problem:
- **What I built:** the actual mechanism, with `file:line` pointers.
- **The trade-off:** what I chose, what I rejected, and why.
- **If asked to extend it:** the one-sentence "here's how I'd add X".
- **Trap I hit / avoided:** the gotcha worth mentioning (link `CLAUDE.md` trap # if relevant).

---

## 1. Large dataset rendering
- **What I built:** `FeatureList.vue` — a side panel that shows the land-price features
...
- **The trade-off:** DOM windowing (`useVirtualizer`) vs. canvas/WebGL rendering.
...
- **If asked to extend it:** for variable-height rows, pass `measureElement` to
...
- **Trap I hit / avoided:** `queryRenderedFeatures` must be called AFTER `setFilter` has
...
## 2. Map integration
- **What I built:** Full-screen MapLibre map over Tokyo in `frontend/app/components/MapView.client.vue`.
...
- **The trade-off:** Raster OSM tiles (zero setup, no key) over vector tiles — fine now;
...
- **If asked to extend it:** add a `source` (GeoJSON) then one+ `layer`s referencing it; update
  data with `source.setData` rather than recreating the layer. That's exactly what step 2 does.
- **Trap I hit / avoided:** MapLibre needs the DOM → component is `.client.vue` so it never SSRs
...
- **The trade-off:** one store for cross-component query state vs. prop drilling or an event bus.
...
- **If asked to extend it:** add the list view reading the same store; add more filters (temp,
  use-category) as more store fields — no component plumbing changes.
- **Trap I hit / avoided:** destructuring a Pinia store drops reactivity — used `storeToRefs`
...
- **What I built:** Two backend modules, each serving the same contract — a GeoJSON
...
- **The trade-off:** Normalize on the backend, not the frontend — every layer reaches Vue in
  ...
  and demos better mid-interview. The normalization lesson is identical either way — we still
  geo-enable it (`[lon, lat]` order, not spoken "lat, lon") and isolate the quirk in one module.

- **If asked to extend it:** add another source module emitting the same `FeatureCollection`
  contract; nothing on the frontend changes except the URL.
- **Trap I hit / avoided:** Open-Meteo is per-point JSON, *not* geospatial — we geo-enable it
  ...
  hot-load into a running BEAM — the server must be restarted (cost us a 500 until then). And
...
- **What I built:** Viewport-driven land-price fetching: `moveend` updates `store.viewportBbox`;
...
- **If asked to extend it:** add a loading state to the store (e.g. `landPriceLoading: ref(false)`)
...
- **Trap I hit / avoided:** `setFilter` takes effect on the next render, not synchronously.
...
- **What I built:** Three-part system: backend fault injection, per-source frontend status,
...
- **The trade-off:**
  ...
  - **Query-param fault injection vs. server-side toggle:** a dedicated `/api/admin/fault`
    ...
    to demo and makes the mechanism explicit in the network tab — ideal for an interview.
...
- **If asked to extend it:** show a timestamp on the error ("weather unavailable since 14:32");
...
- **Trap I hit / avoided:** the weather source must exist before `loadWeatherData` calls
